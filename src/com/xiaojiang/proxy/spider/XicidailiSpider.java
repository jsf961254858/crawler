...
private static boolean isStop = true;
	...
	public XicidailiSpider(ProxyPool proxyPool){
	...
	class CrawlProxyTask extends TimerTask{
		...
		private static final String DOMAIN = "http://www.xicidaili.com";
		...
		private void crawl(int page){
			...
			try {
				...
				for(int i=1;i<size;i++){
					...
					String port = tds.get(3).text().trim();
					String location = tds.get(4).text().trim();
					String type = tds.get(5).text().trim();
					String protocol = tds.get(6).text().trim();
					
					
					//System.out.println(ip+" "+port+" "+location+" "+type+" "+protocol);
